...
- **Agent restart and recovery** -- press `R` to resume agent panes in-place, or to rebuild a recoverable session after tmux or the machine restarts. Press `C` for the same two cases without resuming the previous conversation: a clean restart in place, or a clean rebuild of a recoverable session. Lowercase `r` and `c` do exactly the same work but leave you on the session list instead of attaching, so you can restart several sessions in a row.
...
- **Multi-agent support** -- Claude Code, Codex CLI, OpenCode, Kimi Code, Mistral Vibe, Cursor CLI, Copilot CLI, and Pi.dev
...
Claude Code, Codex CLI, OpenCode, Kimi Code, Mistral Vibe, Cursor CLI, Copilot CLI, and Pi.dev. AoE auto-detects which are installed on your system.
