# Capability Spec: Spike Detection

**Capability**: `spike-detection`
**Created**: 2026-03-24
**Status**: Draft

## Purpose

Prevent false Running status from transient content matches by requiring confirmation across multiple poll cycles. A 1-second confirmation window ensures that brief content spikes (e.g., scrollback artifacts) do not cause spurious Running indications.

## Requirements

### Requirement: Confirm Running detection with a 1-second window
When the detection pipeline first detects `Running` after a non-Running state, the system SHALL hold the previous status for up to 1 second while confirming the Running signal. The status SHALL only commit to `Running` after the signal persists across at least one additional poll cycle.
