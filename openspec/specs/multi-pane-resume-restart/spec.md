# multi-pane-resume-restart Specification

## Purpose
Restarting a session restarts every agent pane it tracks, not only the primary
one. Each pane resumes from the conversation recorded against its own slot, one
pane's failure does not abort its siblings, and the session reports a single
restarting state while the fan-out runs.
## Requirements
