# Capability Spec: Number Jump Session Switching

**Capability**: `number-jump`
**Created**: 2026-03-22
**Status**: Draft

## Purpose

Number jump provides fast session switching by numeric index. Users can press digit keys (1-9) in
the TUI or use `Ctrl+b 1-9` in tmux to jump directly to a session by its position in the list.
Two-digit indices (10-99) are supported via a pending-digit state with auto-confirm.
