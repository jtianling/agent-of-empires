# Capability Spec: Capture Cache

**Capability**: `capture-cache`
**Created**: 2026-03-24
**Status**: Draft

## Purpose

Cache `capture-pane` results with a short TTL to avoid redundant tmux subprocess calls when multiple consumers need pane content within the same poll cycle (e.g., status detection followed by resume token extraction).
