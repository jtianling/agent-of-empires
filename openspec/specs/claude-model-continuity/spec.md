# claude-model-continuity Specification

## Purpose
一个 claude pane 重启后仍然跑在它上一次实际使用的模型上, 而不是回落到 claude 的默认模型.  claude 的 `/model` 是纯 session 内存态, 磁盘上唯一的痕迹是 transcript 里 assistant 条目的 `message.model`, 所以 AoE 从那里探测, 按 slot 持久化, 并在构建启动命令的公共出口注入 `--model`.  探测是纯旁路: 任何失败都保留上一次已知值, 且绝不改变重启行为.
## Requirements
