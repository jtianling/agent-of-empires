# Capability Spec: Activity-Gated Polling

**Capability**: `activity-gated-polling`
**Created**: 2026-03-24
**Status**: Draft

## Purpose

Optimize status polling by skipping expensive `capture-pane` calls when no terminal activity has occurred since the last poll. Uses tmux `window_activity` timestamps to detect changes, with a periodic full check as a safety net.

## Requirements

### Requirement: Skip capture-pane when window activity unchanged
The status poller SHALL track the `window_activity` timestamp for each session across poll cycles. When the current `window_activity` matches the previously recorded value, the poller SHALL skip `capture-pane` and reuse the previous detection result.

#### Scenario: No activity since last poll
- **WHEN** the status poller polls an instance
...
- **THEN** the poller SHALL skip `capture-pane` for that instance
...
- **THEN** the poller SHALL perform a full `capture-pane` and content-based detection
- **AND** update the recorded `window_activity` timestamp

#### Scenario: Periodic full check even without activity
- **WHEN** the status poller polls an instance with no activity change
- **AND** more than 10 seconds have elapsed since the last full capture for that instance
- **THEN** the poller SHALL perform a full `capture-pane` regardless of activity
- **AND** reset the full-check timer

#### Scenario: First poll for a new instance
- **WHEN** the status poller polls an instance for the first time
- **AND** no previous `window_activity` is recorded
- **THEN** the poller SHALL perform a full `capture-pane`
...
- **WHEN** the status poller polls a Claude or Cursor instance
- **AND** the `window_activity` timestamp has not changed
- **THEN** the poller SHALL still read the hook status file
