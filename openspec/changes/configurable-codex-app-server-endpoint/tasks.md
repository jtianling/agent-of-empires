...
- [x] 2.4 Resolver: default when unset, follows a valid value, returns a diagnostic naming the variable and the value when rejected, honors a single-entry list, and refuses a multi-entry one by pointing at the single-URL variable. Saves and restores both variables, and is `#[serial]`.
