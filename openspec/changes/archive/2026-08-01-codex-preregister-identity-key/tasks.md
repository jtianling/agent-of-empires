...
- [x] 3.3 Closed on 2026-08-02. The production `npx @latest` hop ran against the published 0.8.2 with a refreshed cache and no stand-in: a Cross Agent Team Codex session was launched, both panes' daemon rows carried the identity key AoE had injected (compared byte for byte against each pane process's environment, and distinct from each other), `Shift+C` produced `codex-recovery delivered: pane=%94 identity=(aoe, aoe-new)`, and the agent re-registered under its previous team and name with no human input. (npx hop only) Joint drill 2026-07-30 verified the full chain against the locally built daemon: seeding round attached the key to the agent row (after xats fixed an npm-shim double-process ambiguity the drill uncovered), and the acceptance round -- restart, keyed pre-register, zero human input -- produced the daemon's recovery poke and an autonomous re-registration as aoe-codex(aoe) with the same key, new thread, pid-verified pane binding, with AoE's rollout tracking agreeing on the thread id. What remains is only the production `npx @latest` hop, after xats ships `--identity-key-env`: launch a Codex Cross Agent Team session, verify the daemon row carries the identity key, restart with C/R, and verify the daemon pokes the pane and the agent re-registers under its previous `(team, name)`. **Blocked on the xats release; the fallback keeps launches working until then.**
