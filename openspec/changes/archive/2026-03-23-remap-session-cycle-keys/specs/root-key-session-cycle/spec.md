...
The `Ctrl+,` and `Ctrl+.` bindings SHALL follow the established AoE keybinding lifecycle:
- Set up in `setup_session_cycle_bindings()` (works in both nested and non-nested modes)
- Overridden in `apply_managed_session_bindings()` for nested mode with profile-from-option lookup
- Cleaned up in `cleanup_session_cycle_bindings()` and `cleanup_nested_detach_binding()`

#### Scenario: Bindings set in non-nested mode
- **WHEN** `setup_session_cycle_bindings()` is called with a profile
- **THEN** `C-,` and `C-.` SHALL be bound in the root key table with session-guard logic

#### Scenario: Bindings set in nested mode
- **WHEN** `apply_managed_session_bindings()` is called
- **THEN** `C-,` and `C-.` SHALL be bound in the root key table with profile-from-option lookup
...
#### Scenario: Bindings cleaned up in nested hook
- **WHEN** `cleanup_nested_detach_binding()` is called
- **AND** the hook command runs for a non-managed session
- **THEN** the hook command SHALL include `unbind-key -T root C-,` and `unbind-key -T root C-.`
